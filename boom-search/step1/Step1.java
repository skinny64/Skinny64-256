...
public class Step1 {
  ...
  public Step1(final GRBEnv env, final int nbRounds, final int nExtbRounds, final int nExtfRounds, final int regime, final int blocksize) throws GRBException {
    ...
    for (int i = 0; i < 8; i++) {
        ...
        for (int round = 1; round < nbRounds+nExtfRounds; round = round+2) {
        	if (round >= nbRounds) {
                        //v1
                        if (round-nbRounds==5)
                           model.addConstr(DSTKlowExt[round-nbRounds][currentPos/4][currentPos%4], GRB.EQUAL, 0, ""); 
                        else
    ...
    for (int i = 0; i < 8; i++) {
        ...
        for (int round = nExtbRounds-1; round > -1; round = round-2) {
          currentPos = invPermutationTKS[currentPos];
          //v1
          if (round == nExtbRounds-1)
              model.addConstr(DSTKuppExt[round][i/4][i%4], GRB.EQUAL, 0, ""); 
          else
    ...
    model.addConstr(hf, GRB.GREATER_EQUAL, 7, "");
  ...
  private void articleTrails() throws GRBException {
    ...
    // SKINNY-64-256 v1
    else if (blocksize==4 && regime == 4 && nbRounds == 30) {
      maxlanes=1;
      model.addConstr(DXupper[0][3][3], GRB.EQUAL, 1.0, "debug");
      model.addConstr(DXupper[1][0][2], GRB.EQUAL, 1.0, "debug");
      model.addConstr(DXlower[19][0][0], GRB.EQUAL, 1.0, "debug");
