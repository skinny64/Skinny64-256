...
class Constraints_generator():
    ...
    def __init__(self, total_round, initial_round, matching_round, start1, start2):
    ...
    def genConstraints_of_forwardRound(self, r):
        ...
        for i in range(8):
            constr = constr + MITMPreConstraints.fourXOR_forward(input1_round[i],input2_round[i],TK1_1[i],TK1_2[i],TK2_1[i],TK2_2[i],TK3_1[i],TK3_2[i],TK4_1[i],TK4_2[i],\
            input1_sr[i],input2_sr[i],CD_ART[2*i],CD_ART[2*i+1])
            constr = constr + MITMPreConstraints.cd_dual_forward(input1_round[i],input2_round[i],CD_ART[2*i],CD_ART[2*i+1],CD_ART_dual[2*i],CD_ART_dual[2*i+1])
    ...
    def genConstraints_of_backwardRound(self, r):
        ...
        #- Constraints for  ART (input1, input2, TK1_1, TK2_1, TK1_2, TK2_2, TK1_3, TK2_3, inter1_var, inter2_var, output1, output2, cd): 
        for i in range(8):
            constr = constr + MITMPreConstraints.fourXOR_backward(input1_sr[i],input2_sr[i],TK1_1[i],TK1_2[i],TK2_1[i],TK2_2[i],TK3_1[i],TK3_2[i],TK4_1[i],TK4_2[i],\
    ...
    def genConstraints_ini_degree(self):
        ...
        input1_TK4 = Vars_generator.genVars1_TK4(0)
        input2_TK4 = Vars_generator.genVars2_TK4(0)        
    ...
    def genConstraints_additional(self):
        ...
        #constr = constr + ['GDeg1' + ' >= 0']
        #constr = constr + ['GDeg2' + ' >= 0']
        
        constr = constr + [BasicTools.plusTerm(d1_TK1 + d1_TK2 + d1_TK3+ d1_TK4) + ' = 4']
        constr = constr + [BasicTools.plusTerm(d2_TK1 + d2_TK2 + d2_TK3+ d2_TK4) + ' = 4']        
        ...
        if self.ini_r < self.mat_r and self.s1 > self.ini_r and self.s1+6 < self.mat_r and self.s2 > self.mat_r and self.s2+6 < self.TR:
            for i in range(self.TR):
                if i==self.s1 or i==self.s1+2 or i==self.s1+4 or i==self.s1+6 or i==self.s2 or i==self.s2+2 or i==self.s2+4 or i==self.s2+6:
                    constr = constr + [BasicTools.plusTerm(Vars_generator.genVars_ConsumedDeg_of_ART_dual(i))+ ' = 1']
                else:
                    constr = constr + [BasicTools.plusTerm(Vars_generator.genVars_ConsumedDeg_of_ART_dual(i))+ ' = 0']            
        ...
        # for i in range(31):
            # if i==5 or i==7 or i==9 or i==13 or i==15 or i==19 or i==21 or i==25 or i==27 or i==29:
                # constr = constr + [BasicTools.plusTerm(Vars_generator.genVars_ConsumedDeg_of_ART_dual(i))+ ' = 1']
            # else:
                # constr = constr + [BasicTools.plusTerm(Vars_generator.genVars_ConsumedDeg_of_ART_dual(i))+ ' = 0']
        # constr = constr + [BasicTools.plusTerm(Vars_generator.genVars_ConsumedDeg_of_ART_dual(8))+ ' = 1']
        # constr = constr + [BasicTools.plusTerm(Vars_generator.genVars_ConsumedDeg_of_ART_dual(10))+ ' = 1']
        # constr = constr + [BasicTools.plusTerm(Vars_generator.genVars_ConsumedDeg_of_ART_dual(12))+ ' = 1']
        # constr = constr + [BasicTools.plusTerm(Vars_generator.genVars_ConsumedDeg_of_ART_dual(23))+ ' = 1']
        # constr = constr + [BasicTools.plusTerm(Vars_generator.genVars_ConsumedDeg_of_ART_dual(25))+ ' = 1']
        # constr = constr + [BasicTools.plusTerm(Vars_generator.genVars_ConsumedDeg_of_ART_dual(27))+ ' = 1']
        # constr = constr + [BasicTools.plusTerm(Vars_generator.genVars_ConsumedDeg_of_ART_dual(29))+ ' = 1']
    ...
    def genModel(self, filename):
        ...
        fid = open('./skinnye/TR' + str(self.TR) + '_ini' + str(self.ini_r) + '_matr' + str(self.mat_r) + 's1' + str(self.s1) + 's2' + str(self.s2)+ '_v1.lp', 'w')
...
def cmd():
    rd = open('./skinnye/Result_0123.txt', 'w')
    rd.write('TR, ini_r, mat_r, s1, s2' + '\n' )
    for TR in range(31, 32):
        for ini_r in range(1, 2):
            for mat_r in range(16, 17):
                for s1 in range(6, 7):
                    for s2 in range(23, 24):
                        if (ini_r < mat_r and ini_r<s1<mat_r-6 and mat_r<s2) or (ini_r > mat_r and ini_r<s1<TR-6 and s2<mat_r):                        
                            #rd.write(str(TR) + ',' + str(ini_r) + ',' + str(ini_k) + ',' + str(mat_r) + ':')                                        
                            #name = './Model/AES128/V2'
                            name = './skinnye/TR' + str(TR) + '_ini' + str(ini_r) + '_matr' + str(mat_r) + 's1' + str(s1) + 's2' + str(s2)
                            A = Constraints_generator(TR, ini_r, mat_r,s1,s2)
                            A.genModel('')
                            #cmd(4, 1, 7, 4)
                            ...
                            if Model.SolCount == 0:
                                ...
                                rd.write(str(TR) + ',' + str(ini_r) + ',' + str(mat_r) + ',' + str(s1) + ',' + str(s2)) 
                                #rd.write(str(str(Sol['GMat']) + '\n')
                                rd.flush()
